...
export default function Orders() {
  ...
  const copy = async (o) => {
    const items = o.items.map((i) => `  - ${i.name} (Size ${i.size} x${i.quantity})${i.shoeSize ? ` [Foot: US ${i.shoeSize}]` : ''}`).join('\n');
    const text =
            ...
            {orders.map((o) => (
              <tr key={o._id} className="border-t border-slate-100 align-top">
                ...
                <td className="p-3 text-slate-600">
                  {o.items.map((i, idx) => (
                    <div key={idx}>{i.name} <span className="text-slate-400">({i.size} x{i.quantity})</span>{i.shoeSize && <span className="text-gold ml-1">Foot: US {i.shoeSize}</span>}</div>
                  ))}
                </td>
                <td className="p-3">
                  <select
